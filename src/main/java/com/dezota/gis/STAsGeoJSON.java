...
/*
 *
 * @summary Convert geometry to GeoJSON text
 *
 * @usage SELECT ST_AsGeoJSON(ST_GeomFromEWKB(the_geom)) FROM "cartodb".acs."acs_2019_5yr_place_49_utah"
 *
...
public class STAsGeoJSON implements SimpleFunction {
    ...
    public void eval() {
        ...
        String outJson = geom1.asGeoJson();

        int outputSize = outJson.getBytes().length;
        buffer = out.buffer = buffer.reallocIfNeeded(outputSize);
        out.start = 0;
        out.end = outputSize;
        buffer.setBytes(0, outJson.getBytes());
    }
}
