...
public class STGeomFromEWKB implements SimpleFunction {
    ...
    public void eval() {
        ...
        String ewkbText = com.dezota.gis.FunctionHelpers.toStringFromUTF8(input.start, input.end,
                input.buffer);
        ...
        byte[] ewkbByteArr = com.dezota.gis.FunctionHelpers.hexToBytes(ewkbText);
        java.nio.ByteBuffer ewkbBB = java.nio.ByteBuffer.wrap(ewkbByteArr);
        byte[] wkbByteArr = com.dezota.gis.FunctionHelpers.hexToBytes(wkbText);
        ...
        if (byteOrderWKB == com.dezota.gis.WKBConstants.wkbNDR) {
            ewkbBB.order(java.nio.ByteOrder.LITTLE_ENDIAN);
            wkbBB.order(java.nio.ByteOrder.LITTLE_ENDIAN);
        } else if (byteOrderWKB == com.dezota.gis.WKBConstants.wkbXDR) {
        ...
        if (geomType >= com.dezota.gis.WKBConstants.wkbPoint && geomType <= com.dezota.gis.WKBConstants.wkbGeometryCollection) {
            if (hasSRID) {
